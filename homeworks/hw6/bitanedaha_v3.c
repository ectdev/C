...
void get_ingredients(Ingredient* ingr) {
  ...
  for (i = 0; i < num_ingredients; i++) {
    ...
    int total_percent = ingredients[i].carbs + ingredients[i].proteins + ingredients[i].fats;
    if (total_percent < 100) {
      ingredients[i].carbs += 100 - total_percent;
    }
    ingredients[i].calories = (ingredients[i].weight * ingredients[i].carbs * CARB_CAL_1G) / 100
                              + (ingredients[i].weight * ingredients[i].proteins * PROTEIN_CAL_1G) / 100
                              + (ingredients[i].weight * ingredients[i].fats * FAT_CAL_1G) / 100
                              + (ingredients[i].weight * (100 - total_percent) * WATER_CAL_1G) / 100;
